...
export function Hero() {
  ...
  // Scroll-out parallax: track how far the section has exited the viewport top
  const { scrollYProgress } = useScroll({
    target: sectionRef,
    offset: ["start start", "end start"],
  });

  // Text drifts upward faster — feels closer, lighter
  ...
  // Graph drifts upward slower — feels heavier, further back
  const graphParallaxY = useTransform(
    scrollYProgress,
    [0, 1],
    reduced ? [0, 0] : [0, -20],
  );

  // Mouse-follow 3D tilt for the graph card
  const mouseX = useMotionValue(0);
  const mouseY = useMotionValue(0);
  const tiltY = useSpring(
    useTransform(mouseX, [-0.5, 0.5], reduced ? [0, 0] : [-5, 5]),
    { stiffness: 300, damping: 28 },
  );
  const tiltX = useSpring(
    useTransform(mouseY, [-0.5, 0.5], reduced ? [0, 0] : [5, -5]),
    { stiffness: 300, damping: 28 },
  );
  ...
  return (
    ...
    >
      ...
      <div className="relative mx-auto max-w-6xl px-6">
        {/* Text content — parallaxes out faster than the graph */}
        ...
        {/* Graph — parallaxes out slower (heavier, further back) */}
        <motion.div
          style={{ y: graphParallaxY }}
        ...
        >
          ...
          {/* Floating side cards — outside tilt so they stay flat */}
          <motion.div
