...
def test_dataset_pipeline_cli_runs_end_to_end(tmp_path, monkeypatch, capsys) -> None:
    ...
    assert (output_dir / "gemma4-run" / "reasoner_attempts.jsonl").exists()
    assert (output_dir / "gemma4-run" / "findings.json").exists()
    assert (output_dir / "gemma4-run" / "impact_paths.json").exists()
    assert (output_dir / "gemma4-run" / "triage_backlog.json").exists()
    assert (output_dir / "gemma4-run" / "mcp_context.json").exists()
    assert (output_dir / "gemma4-run" / "product_summary.json").exists()
